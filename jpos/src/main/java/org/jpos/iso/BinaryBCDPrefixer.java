...
/**
 * BinaryBCDPrefixer constructs a prefix storing the length in binary.
 *
 * @author joconnor
 * @version $Revision$ $Date$
 */
public class BinaryBCDPrefixer implements Prefixer
{
    /**
     * A length prefixer for up to 255 chars. The length is encoded with 1 unsigned byte.
     */
    public static final BinaryBCDPrefixer B = new BinaryBCDPrefixer(1);

    /**
     * A length prefixer for up to 65535 chars. The length is encoded with 2 unsigned bytes.
     */
    public static final BinaryBCDPrefixer BB = new BinaryBCDPrefixer(2);

    /** The number of digits allowed to express the length */
    private int nBytes;

    public BinaryBCDPrefixer(int nBytes)
    {
        this.nBytes = nBytes;
    }
